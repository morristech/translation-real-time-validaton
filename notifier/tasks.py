...
from . import compare, mailer, wti, validators
from .segment import build_translation_segment
...
EMAIL_TOPIC = 'Translations not passing the validation test - file {}, language {}, string_id {}'
...
def validate_translation(wti_key, mail_client, translation_payload, content_type):
    segment = build_segment_from_payload(wti_key, translation_payload, content_type)
    ...
    if not validator.validate():
        if validator.notify_agent:
            topic = EMAIL_TOPIC.format(filename, other_locale, string_id)
            logger.info(topic)
            result = yield from mailer.send(mail_client, segment.user_email,
                                            validator.diff_errors, validator.url_errors,
                                            segment.content_type, topic)
            if result:
        ...
        if validator.change_status:
            if segment.user_role != 'manager':
                yield from translate.change_status(wti_key,
                                                   segment.locale, segement.string_id, segment.content)
